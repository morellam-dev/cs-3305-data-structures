...
// Precondition: k> 0.
// Postcondition: The function assumes that k is the index of an array element,
// where the array represents a complete binary tree. The return value is the
// index of the parent of node k, using the formula from rule 3 on page 624.

size_t left_child(size_t k);
// Postcondition: The function assumes that k is the index of an array element,
// where the array represents a complete binary tree. The return value is the
// index of the left child of node k, using the formula from rule 2 on page 624.

size_t right_child(size_t k);
// Postcondition: The function assumes that k is the index of an array element,
// where the array represents a complete binary tree. The return value is the
// index of the right child of node k, using the formula from rule 2 on page
// 624.
...
// elements form a heap **except** that data[0] may be in an incorrect location.
// location.
// Postcondition: The data values have been rearranged so that the first n
// elements of data now form a heap.
...
//
{
  ...
  size_t
      k;  // Index of new element as it is being pushed upward through the heap
...
// Library facilities used: itemtool.h (from page 277), cstdlib
{
  ...
  size_t big_child_index;  // Index of the larger child of the node that's
                           // moving down
  bool heap_ok = false;    // Will change to true when the heap becomes correct

  current = 0;

  // Note: The loop keeps going while the heap is not okay, and while the
  // current node has at least a left child. The test to see whether the current
  // node has a left child is left_child(current) < n.
  while ((!heap_ok) && (left_child(current) < n)) {
    ...
    // Check whether the larger child is bigger than the current node. If so,
    // then swap the current node with its bigger child and continue; otherwise
    // we are finished.
