...
double test1() {
  ...
  cout << "I will now test the default constructor. After initializing\n"
       ...
       << "function should always return zero (even beyond MAX_EX)." << endl;
  ...
  cout << "I will now test constructor like this: polynomial p(0.0, 3).\n"
       ...
       << "should have all zero coefficients (even beyond MAX_EX)." << endl;
  ...
  cout << "Checking that add_to_coef correctly adjusts degree downward."
       << endl;
  p.assign_coef(1.0, 10);
  p.add_to_coef(-p.coefficient(10), 10);
  if (!correct(p, 2, c)) return 0;

  cout << "Checking that assign_coef correctly adjusts degree downward."
       << endl;
  p.assign_coef(1.0, 10);
  p.assign_coef(0, 10);
  if (!correct(p, 2, c)) return 0;
...
double test4() {
  ...
  for (i = 1; i <= TESTSIZE; ++i) {
    if ((i != (TESTSIZE / 2) + 1) && (p.previous_term(i) != i - 1)) return 0;
  }
  cout << "First for" << endl;
  if (p.previous_term((TESTSIZE / 2) + 1) != (TESTSIZE / 2) - 1) return 0;
  cout << "0 term skipped" << endl;
  for (i = TESTSIZE + 1; i < 2 * polynomial::MAX_EX; ++i) {
    if (p.previous_term(i) != TESTSIZE) return 0;
  }
  cout << "Second for" << endl;
  p.assign_coef(0, 0);
  if (p.previous_term(1) != UINT_MAX) return 0;
  cout << "run off bottom" << endl;
  cout << "Testing previous_term(0)" << endl;
  if (p.previous_term(0) != UINT_MAX) return 0;
...
double run_a_test(int number, const char message[], double test_function(),
                  int max) {
  double result;
