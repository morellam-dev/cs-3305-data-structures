 ...
 */
template <class T>
binary_tree_node<T> *build_from_node_lists(const T preorder[],
                                           const T inorder[], size_t pre_start,
                                           size_t pre_end, size_t in_start,
                                           size_t in_end) {
  ...
  binary_tree_node<T> *left = build_from_node_lists(
      preorder, inorder, pre_start + 1, pre_start + lsize + 1, in_start, r);
  binary_tree_node<T> *right = build_from_node_lists(
      preorder, inorder, pre_start + lsize + 1, pre_end, r + 1, in_end);
